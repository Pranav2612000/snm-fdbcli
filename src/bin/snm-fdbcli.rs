...
async fn cmd_repl(db: &Database) -> FdbResult<()> {
    ...
    loop {
        ...
        let result = match cmd {
            ...
            other => {
                println!("Unknown command: {other}. Type 'help' for list.");
                Ok(())
            }
        };
